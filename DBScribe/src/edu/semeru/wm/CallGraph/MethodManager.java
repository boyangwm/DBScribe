...
public class MethodManager {
	...
	public void addMethod(String m){
		...
		if(methodList == null){
			...
			String packageName = splitm[0].substring(0, splitm[0].lastIndexOf("."));
			String className = splitm[0].substring(splitm[0].lastIndexOf(".")+1);
			String methodName = splitm[1];
			int numOfPara = Integer.parseInt(splitm[2]);
			Method newMethod = new Method(packageName, className, methodName, numOfPara);
			//update key
